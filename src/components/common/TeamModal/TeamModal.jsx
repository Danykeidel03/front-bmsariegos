...
const TeamModal = ({ isOpen, onClose }) => {
  ...
  const handleSaveName = async (teamId) => {
    try {
      ...
      console.log(error);
      const errorMessage =
        error.response?.status === 404 ? 'Equipo no encontrado' : 'Error del servidor';
            ...
            teams.map((team, index) => (
              <div key={team._id} className="team-item">
                ...
                <div className="team-actions">
                  {editingTeam !== team._id && (
                    <button className="edit-btn" onClick={() => handleEditName(team)}>
                      Editar
                    </button>
                  )}
                  <button className="delete-btn" onClick={() => handleDelete(team._id)}>
                    Eliminar
                  </button>
                </div>
              </div>
