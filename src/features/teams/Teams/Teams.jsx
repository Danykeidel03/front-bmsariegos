...
import apiClasificaciones from '../../../api/apiClasificaciones';
import apiMatch from '../../../api/apiMatch';
import apiRival from '../../../api/apiRival';
import OptimizedImage from '../../../components/ui/OptimizedImage/OptimizedImage';
import LocalOptimizedImage from '../../../components/ui/LocalOptimizedImage/LocalOptimizedImage';
import './Teams.css';
...
const Teams = () => {
  ...
  const getClasificacionByTeam = (teamName) => {
    return Object.values(clasificaciones).find(
      (clasificacion) => clasificacion.teamName === teamName
    );
  };
  ...
  const getCompletedMatchesByTeam = (teamId) => {
    return matches
      .filter((match) => match.completed === 1 && match.ownTeam._id === teamId)
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="clasificacion-info">
                            <h3>
                              {clasificacion.category} - {clasificacion.division}
                            </h3>
                            <div className="clasificacion-table">
                              ...
                              {clasificacion.clasificacion.map((equipo) => (
                                ...
                                >
                                  ...
                                  <span
                                    style={
                                      equipo.nombre.toLowerCase().includes('sariegos') ||
                                      equipo.nombre === 'CASA DEL PUEBLO BMS ZARDINO'
                                        ? { fontWeight: 'bold' }
                                        : {}
                                    }
                                  >
                                    {cleanTeamName(equipo.nombre)}
                                  </span>
