...
```javascript
const prefetchProps = usePrefetchOnInteraction(() => import('./ExpensiveComponent'));
...
const prefetchProps = usePrefetchOnInteraction(() => import('./pages/Contact/Contact'));

<Link to="/contacto" {...prefetchProps}>
  Contacto
</Link>;
```
